...
class GraphByMatrix(SingleChoice):
    def generate(self):
        ...
        self.text ='В таблице приведена стоимость перевозки между ' +\
...
class LightPanel(SingleChoice):
    def generate(self):
        ...
        self.text = f'На световой панели в ряд расположены {n} лампочек. ' +\
                    f'Каждая из первых {first} лампочек может гореть красным, жёлтым или зелёным цветом. ' +\
                    f'Каждая из остальных {last} лампочек может гореть одним из двух цветов — красным или белым. ' +\
